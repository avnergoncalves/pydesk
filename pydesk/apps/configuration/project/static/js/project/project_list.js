$(function() {
	...
	$("#grid_list_project").grid(
			{
				url : "/configuration/project/ajax/list",
				data:{
					find: $('#find'),
					status: $('#status')
				},
						...
						},
						{
							...
							acoes : {
								"editar" : function(res, row_id) {
									window.location.href = "/configuration/project/edit?id=" + res;
								}
							}
						},
						{
	...
	$('#status').change(function(e){
		$('#grid_list_project').grid().reload({consultar:true});
	});
	...
	$('#btn_active').on('click', function(){
		if(confirm('Deseja ativar os itens selecionados ?')){
			$('#hdd_is_active').val('1');
			$.post("/configuration/project/ajax/toogle_status", $('#form_list_project').serialize())
